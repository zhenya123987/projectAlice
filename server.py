...
def handle_dialog(req, res):
    ...
    if req['session']['new']:
        ...
        if flag:
            res['response']['text'] = f'Привет! Купи {rabbit}а!'
        else:
            res['response']['text'] = f'Привет! Купи {elef}а!'
        # Получим подсказки
    ...
    if req['request']['original_utterance'].lower() in [
        ...
        if flag:
            res['response']['text'] = f'{rabbit}а можно найти на Яндекс.Маркете!'
            flag = True
        else:
            res['response']['text'] = f'{elef}а можно найти на Яндекс.Маркете!'
        res['response']['end_session'] = True
        return

        # Если нет, то убеждаем его купить слона!
    if flag:
        res['response']['text'] = f"Все говорят '{req['request']['original_utterance']}', а ты купи {rabbit}a!"
    else:
        res['response']['text'] = f"Все говорят '{req['request']['original_utterance']}', а ты купи {elef}a!"
    res['response']['buttons'] = get_suggests(user_id)
...
def get_suggests(user_id):
    ...
    if len(suggests) < 2:
        if flag:
            suggests.append({
                "title": "Ладно",
                "url": f"https://market.yandex.ru/search?text={rabbit}",
                "hide": True
            })
        else:
            suggests.append({
                "title": "Ладно",
                "url": f"https://market.yandex.ru/search?text={elef}",
                "hide": True
            })
